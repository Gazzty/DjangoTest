...
class ThreadTestCase(TestCase):
    def setUp(self):
        ...
        self.user2 = User.objects.create_user('user2', None, 'user1234')
    ...
    def test_add_messages_to_thread(self):
        self.thread.users.add(self.user1, self.user2)
        message1 = Message.objects.create(user=self.user1, content='Holis')
        message2 = Message.objects.create(user=self.user2, content='Q ondaaaa')
    ...
    def test_find_thread_with_custom_manger(self):
        self.thread.users.add(self.user1, self.user2)
        ...
        self.assertIsNotNone(self.thread, threads)